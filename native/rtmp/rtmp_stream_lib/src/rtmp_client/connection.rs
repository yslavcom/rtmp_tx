...
use mio::Poll;
use mio::Token;
use mio::Ready; // Removed in newer version
use mio::PollOpt; // Removed in newer version
...
impl Connection {
    pub fn new(
        ...
        socket
            .set_send_buffer_size(SOCKET_SEND_BUFFER_SIZE)
            .expect("Could not set send buffer size");
        ...
        match self.socket.read(&mut buffer) {
            ...
            Err(error) => {
                if error.kind() == io::ErrorKind::WouldBlock {
                    ...
                    println!("Failed to send buffer with error {}", error);
        ...
        match self.socket.write_all(&bytes) {
            ...
            Err(error) => {
                if error.kind() == io::ErrorKind::WouldBlock {
                    ...
                    println!("Failed to send with error {}", error);
                    return Err(error);
                }
            }
        };
